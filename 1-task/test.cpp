...
void print_results(matrix &A, vector<double> &b, vector<double> &expected_x,
...
int main() {
  ...
  //        };
  //        vector<double> b = {2, 1, 1};
  //        vector<double> expected_x = {-99801. / 100798, 49850. / 50399, 500.
  //        / 499}; vector<double> actual_x = LU(A, b);
  //
  //        print_results(A, b, expected_x, actual_x);
  //    }
  //
  ...
  //        vector<double> expected_x = {-997909201. / 1007980000, 9970001. /
  //        10079800, 9999. / 9980}; vector<double> actual_x = LU(A, b);
  //
  //        print_results(A, b, expected_x, actual_x);
  //    }
  //
  ...
  //        vector<double> expected_x = {11099. / 121200, -100001. / 202000,
  //        49501. / 60600}; vector<double> actual_x = LU(A, b);
  //
  //        print_results(A, b, expected_x, actual_x);
  //    }
  //
  ...
  vector<pair<double, double>> params = {{1, 10},    {1, 100}, {1, 1000},
                                         {1, 10000}, {0.1, 1}, {0.01, 1},
                                         {0.001, 1}};
  ...
  for (auto &pair : params) {
    ...
    double error = norm(subtract(x_tilde, x_starred)),
           zeta = error / norm(x_starred), rho = norm(r) / norm(f);

    cout << setprecision(10) << alph << '\t' << beta << '\t' << norm(A) << '\t'
         << norm(A_inv) << '\t' << norm(A) * norm(A_inv) << '\t' << error
